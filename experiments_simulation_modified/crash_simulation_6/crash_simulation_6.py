...
import random
import numpy as np
from experiments_simulation_modified.crash_simulation_6.crash_simulation_helper import getV1BeamNGCoordinaes, getV2BeamNGCoordinaes
from experiments_simulation_modified.crash_simulation_6.crash_simulation_helper import AngleBtw2Points, getDistance
from experiments_simulation_modified.crash_simulation_6.vehicle_state_helper import DamageExtraction, DistanceExtraction, RotationExtraction
import csv
import sys
...
scenario = Scenario('GridMap', 'crash_simulation_6')
...
IMPACT_POSITION_X = 212
IMPACT_POSITION_Y = 127

# roads for striker and victim vehicle.
road_striker = [(217, 182), (212, 127)]
road_victim = [(171, 131), (212, 127)]

actual_striker_damage = "F"
actual_victim_damage = "L"
...
def generateRandomPopulation(N=5,Gene=14):
    ...
    initial_population = []
    initial_population.append([1, 0, 3, 1, 0, 2, 5, 3, 1, 5, 0, 1, 5, 1])
    initial_population.append([0, 2, 1, 5, 6, 3, 7, 4, 2, 3, 1, 3, 3, 7])
    initial_population.append([4, 4, 6, 7, 2, 7, 6, 8, 6, 2, 2, 4, 8, 1])
    initial_population.append([1, 7, 8, 5, 5, 6, 4, 6, 6, 2, 6, 7, 7, 4])
    initial_population.append([4, 5, 4, 8, 3, 4, 8, 1, 2, 4, 5, 2, 4, 3])
...
def generateRandomPopulation(N=5,Gene=10):
    ...
    initial_population = []
    initial_population.append([1, 1, 3, 1, 0, 2, 5, 3, 1, 5, 1, 1, 5, 1])
    initial_population.append([1, 2, 1, 5, 6, 3, 7, 4, 2, 3, 1, 3, 3, 7])
    initial_population.append([4, 4, 6, 7, 2, 7, 6, 8, 6, 2, 2, 4, 8, 1])
    initial_population.append([1, 7, 8, 5, 5, 6, 4, 6, 6, 2, 6, 7, 7, 4])
    initial_population.append([4, 5, 4, 8, 3, 4, 8, 1, 2, 4, 5, 2, 4, 3])
    initial_population = convertPopulation(initial_population)
    return initial_population
...
for population in populations:
    ...
    scenario.add_vehicle(vehicleStriker, pos=(striker_points[0][0], striker_points[0][1], 0),
                         rot=(0, 0, 0))  # get car heading angle
    scenario.add_vehicle(vehicleVictim, pos=(victim_points[0][0], victim_points[0][1], 0), rot=(0, 0, 270))
    ...
    try:
        ...
        for number in range(60):
            ...
            if vehicleStriker.state['pos'][0] > 212 and vehicleStriker.state['pos'][1] > 127:
                # print('free state')
            ...
            # print(vehicleStriker.state['pos'])
            if vehicleVictim.state['pos'][0] > 212 and vehicleVictim.state['pos'][1] > 127:
                # print('free state')
...
for _ in range(20):  # Number of Generations to be Iterated.
    ...
    for children in next_population:
        ...
        scenario.add_vehicle(vehicleVictim, pos=(victim_points[0][0], victim_points[0][1], 0),
                             rot=(0, 0, 270))  # get car heading anlge
        ...
        try:
            ...
            for number in range(60):
                ...
                sensors = bng.poll_sensors(vehicleStriker)  # Polls the data of all sensors attached to the vehicle
                # print(vehicleStriker.state['pos'])
                if vehicleStriker.state['pos'][0] > 211 and vehicleStriker.state['pos'][1] > 126:
                    # print('free state')
                    vehicleStriker.control(throttle=0, steering=0, brake=0, parkingbrake=0)
                    vehicleStriker.update_vehicle()
                ...
                sensors = bng.poll_sensors(vehicleVictim)  # Polls the data of all sensors attached to the vehicle
                # print(vehicleStriker.state['pos'])
                if vehicleVictim.state['pos'][0] > 213 and vehicleVictim.state['pos'][1] > 126:
                    # print('free state')
                    vehicleVictim.control(throttle=0, steering=0, brake=0, parkingbrake=0)
                    vehicleVictim.update_vehicle()
